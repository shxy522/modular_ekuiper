...
def apply_chebyshev_filter(data, sample_rate, filter_band, ftype, order=5, rp=3):
    """
    使用切比雪夫滤波
    :param data:用户输入信号数据
    :param fs:采样率
    ...
    if ftype == 0:
        cut = filter_band / nyq
        b, a = signal.cheby1(order, rp, cut, btype=type_meaning[ftype])
    elif ftype == 1:
        ...
        b, a = signal.cheby1(order, rp, [lowcut, highcut], btype=type_meaning[ftype])
    elif ftype == 3:
...
class Chebyshev(Function):
    ...
    def exec(self, args: List[Any], ctx: Context):
        ...
        return apply_chebyshev_filter(args[0], args[1], args[2], args[3], args[4], args[5]).tolist()
